...
class SavingRecord {
  ...
  static async isExist({ user_id, saving_plan_id }) {
    const saving_record = await connection.promise().query(
      'SELECT * FROM saving_records WHERE user_id = ? AND saving_plan_id = ?',
      [user_id, saving_plan_id],
    ).catch((err) => console.log(err));
  ...
  static async softDelete(id) {
    const saving_record = await connection.promise().query(
      'UPDATE saving_records SET deleted_at = NOW() WHERE id = ?',
      [id],
    ).catch((err) => console.log(err));
    return saving_record[0];
  }
